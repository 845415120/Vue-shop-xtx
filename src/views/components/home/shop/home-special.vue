<template>
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="homeSpecial">
        <div class="special-item" v-for="item in brands" :key="item.id">
        <RouterLink to="/">
          <img :src="item.cover" alt />
          <div class="meta">          
             <p class="title">{{item.title}}
              <small class="">{{item.summary}}</small>
            </p>              
...
export default {
  ...
  setup(){
  const brands = ref([])
  axios.get('https://pcapi-xiaotuxian-front.itheima.net/home/special').then(res=>{
